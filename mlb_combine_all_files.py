...
def main():
    ...
    # 2) Build lookups (teams, weather, betting, matchups)
    team_map = { normalize(g[k]): g[k] for g in starters for k in ("home_team","away_team") }
    ...
    # ensure every starter has matchup
    for g in starters:
        for side in ("home_team","away_team"):
            t, n = g[side], normalize(g[side])
            if n not in matchup:
    ...
    # 3) Append each player-game to archive, collect structured output
    with archive.open("a", encoding="utf-8") as arch:
        for r in rosters:
            pid = str(r["player_id"]); name = r["player"]; team = r["team"]
            ...
            starters_names = { normalize(g["home_pitcher"]) for g in starters } \
                           | { normalize(g["away_pitcher"]) for g in starters }
            is_st = normalize(name) in starters_names
