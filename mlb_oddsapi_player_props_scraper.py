...
print(f"📡 Requesting player props from Odds API with market(s): {MARKET}")
...
data = response.json()
if not data:
    print("⚠️ No rows collected. Possibly no data yet for today.")
    exit(0)
...
if not rows:
    print("⚠️ No rows collected. Possibly no valid props found for this market.")
    exit(0)
